...
This program reads a bunch of log files generated by a program and sends the interesting messages to stdout. It supports
log files rotated and optionnaly compressed by logrotate or another similar mecanism.
...
The following configuration file will match any line containing the FATAL, ERROR or WARNING strings against all files
in the directory "/var/log/postgres" matching "postgresql\.log" (eg. postgresql.log, postgresql.log.1, postgresql.log.2.gz, ...).
...
# It writes to stdout the lines matching regexps from the file given as parameter
sub read_files
{
    my ($reffilelist)=@_;
